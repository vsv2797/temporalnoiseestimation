...
def evaluate_residual(arc, smoothed):
    ...
    var = np.var(residual)
    smoothness = np.var(arc) - np.var(smoothed)
    score = (coh * smoothness) / var if var > 0 else 0
    return coh, var, smoothness, score
...
for cutoff in cutoffs:
    for order in orders:
        try:
            ...
            coh, var, smoothness, score = evaluate_residual(arc_phase, smoothed)
            bw_results.append((cutoff, order, coh, var, smoothness, score))
        except Exception:
            bw_results.append((cutoff, order, 0, np.inf, 0, 0))

# Savitzky-Golay evaluation (with smoothing check)
sg_results = []
window_lengths = [7, 9, 11]
poly_orders = [2, 3]
...
for win in window_lengths:
    for poly in poly_orders:
        try:
            ...
            coh, var, smoothness, score = evaluate_residual(arc_phase, smoothed)
            sg_results.append((win, poly, coh, var, smoothness, score))
        except Exception:
            sg_results.append((win, poly, 0, np.inf, 0, 0))

# Save results
bw_df = pd.DataFrame(bw_results, columns=["cutoff", "order", "coherence", "residual_var", "smoothness_gain", "score"])
sg_df = pd.DataFrame(sg_results, columns=["window_length", "poly_order", "coherence", "residual_var", "smoothness_gain", "score"])

bw_df.to_csv("bw_filter_eval_v2.csv", index=False)
sg_df.to_csv("sg_filter_eval_v2.csv", index=False)

print("Evaluation complete. Saved: bw_filter_eval_v2.csv and sg_filter_eval_v2.csv")
